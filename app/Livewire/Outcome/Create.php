...
class Create extends Component
{
    ...
    public function getUpdatedReference()
    {
        $this->stockEntry = StockEntry::select('stock_entries.*')
        ->selectRaw('SUM(batches.qty * CAST(batches.new_price AS INTEGER)) AS nominal')
        ->leftJoin('batches', 'stock_entries.id', '=', 'batches.stock_entry_id')
        ...
        ->groupBy('stock_entries.id')
        ->get();
        $this->dispatch('updatedReference', stockEntry: $this->stockEntry);
    }
    ...
    {
        return view('livewire.outcome.create', [
    ...
    public function save()
    {
        ...
        try {
            ...
            $newOutcome->category = $this->category;
            $newOutcome->payment_method = $this->payment_method;
            $newOutcome->branch_id = $this->branch_id;
            $newOutcome->date = Carbon::parse($this->date);
            $newOutcome->note = $this->note;
            $newOutcome->status = $this->status;
            $newOutcome->account_id = $this->account_id;
            $newOutcome->supplier_id = $this->supplier_id;
            $newOutcome->nominal = (int)filter_var($this->nominal, FILTER_SANITIZE_NUMBER_INT);
            $newOutcome->code = $code;
            $newOutcome->save();

            $newAttributes = collect();
            if ($this->category == 'Obat & Alkes' && $this->references != []) {
                foreach ($this->references as $key => $r) {
                    $newDetail = new DetailOutcome();
                    $newDetail->outcomes_id = $newOutcome->id;
            ...
            $log->model = 'Outcome';
            $log->model_id = $newOutcome->id;
            $log->log = 'Telah membuat Pengeluaran dengan Detail Pengeluaran ' . $newAttributes->join(', ');
            $log->save();
            DB::commit();

            Toaster::success('Data Pengeluaran berhasil disimpan');
            return $this->redirectRoute('outcome.index', navigate: true);
        } catch (\Throwable $th) {
            DB::rollback();
            dd($th);
            $this->notification = 'Gagal simpan, silakan coba beberapa saat lagi';
