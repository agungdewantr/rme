...
<x-page-layout >
        ...
        <div class="card-body">
            <div class="row">
                {{-- <div class="col-md-6">
                    <div class="col-md-12">
                        <div class="mb-3">
                            <label for="exampleFormControlTextarea1 h6" class="form-label">Penyakit Sekarang/Keluhan
                                Saat ini</label>
    ...
    @if ($medicalRecord->weight && $medicalRecord->height)
        @if (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 40.0)
            bmi_status:'Obese Class III',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 35.0 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 39.99)
            bmi_status:'Obese Class II',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 30.0 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 34.99)
            bmi_status:'Obese Class I',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 25.0 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 29.99)
            bmi_status:'Overweight',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 18.5 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 24.99)
            bmi_status:'Normal',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 17.0 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 18.49)
            bmi_status:'Underweight',
        @elseif (round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) >= 16.0 && round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) <= 16.99)
            bmi_status:'Severely Underweight',
        @else
        bmi_status:'Very Severely Underweight',
        bmi: '{{ round($medicalRecord->weight / pow($medicalRecord->height / 100, 2), 2) }}',
        @endif
    @else
        bmi_status: 'Very Severely Underweight',
        bmi: '0',
    ...
    weight: @entangle('weight'),
        ...
        hpht: @entangle('hpht'),
        interpretation_childbirth: '{{ \Carbon\Carbon::parse($medicalRecord->hpht)->addDays(280)->format('d-m-Y') }}',
        age_childbirth: '{{ floor(\Carbon\Carbon::now()->diffInWeeks(\Carbon\Carbon::parse($medicalRecord->hpht))) }} minggu {{ floor(\Carbon\Carbon::now()->diffInDays(\Carbon\Carbon::parse($medicalRecord->hpht))) % 7 }} hari',
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            {{-- <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('gravida') is-invalid @enderror"
                                    id="gravida" wire:model="gravida">
                                @error('gravida')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                        ...
                        <div class="col-md-2">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('hpht') is-invalid @enderror"
                                    id="hpht" x-model="hpht"
                                    x-on:change="(e)=>{
                                        let hphtReversed = e.target.value.split('-').reverse().join('-');
                                        let date = dayjs(hphtReversed).add(280, 'day')
                                        interpretation_childbirth=date.format('DD-MM-YYYY')
                                        edd = date.format('YYYY-MM-DD')
                                        let weeks = dayjs().diff(dayjs(hphtReversed), 'week');
                                        let days = dayjs().diff(dayjs(hphtReversed), 'day') - weeks * 7;
                        ...
                        {{-- <div class="col-md-4">
                            <div class="mb-3" wire:ignore>
                                ...
                                <select name="illness_history" id="illness_history" multiple
                                    class="select2 form-control w-100">
                                    @foreach ($medicalRecord->illnessHistories as $item)
                        ...
                        <div class="col-md-4">
                            <div class="mb-3" wire:ignore>
                                ...
                                <select name="allergy_history" id="allergy_history" wire:model="allergy_history"
                                    ...
                                    @foreach ($medicalRecord->allergyHistories as $item)
                                        <option value="{{ $item->name }}">{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <div class="">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" value=""
                                                id="flexCheckDefault" x-model="showLab">
                                    ...
                                    <div class="table-responsive" x-show="showLab">
                                        <table class="table">
                                            <tr>
                                                <th>Pemeriksaan</th>
                                                <th>Hasil</th>
                                ...
                                <div class="mb-3">
                                 ...
                                 <div class="mb-3">
                                    ...
                                    <input type="date"
                                        ...
                                        id="next_control" wire:model="next_control">
                                    @error('next_control')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
