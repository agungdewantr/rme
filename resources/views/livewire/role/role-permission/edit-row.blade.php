<tr>
    <td style="width:250px">
        <select name="feature" wire:model.live="feature" class="form-select" disabled>
    ...
    <td style="width:250px; vertical-align:middle" wire:ignore>
        <select name="role_id" wire:model.live="role_id" id="{{ $id }}" class="form-select" disabled>
            <option value="">Pilih Role</option>
            @foreach ($roles as $item)
                <option value="{{ $item->name }}" {{ $item->name === $role_id ? 'selected' : '' }}>{{ $item->name }}
                </option>
            @endforeach
        </select>
    </td>
    ...
    <td>
        <div class="d-flex">
            <button class="btn btn-sm btn-warning me-2" wire:dirty wire:click="saveEdit">
