@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="tw-flex tw-items-center tw-mt-2 tw-justify-end">
        <div class="tw-flex tw-justify-end tw-flex-1 tw-sm:hidden">
            ...
            @else
                <a wire:navigate href="{{ $paginator->previousPageUrl() }}" class="tw-relative tw-inline-flex tw-items-center tw-px-4 py-2 tw-text-sm tw-font-medium tw-text-gray-700 tw-bg-white tw-border tw-border-gray-300 tw-leading-5 tw-rounded-md tw-hover:text-gray-500 tw-focus:outline-none tw-focus:ring tw-ring-gray-300 tw-focus:border-blue-300 tw-active:bg-gray-100 tw-active:text-gray-700 tw-transition tw-ease-in-out tw-duration-150 tw-dark:bg-gray-800 tw-dark:border-gray-600 tw-dark:text-gray-300 tw-dark:focus:border-blue-700 tw-dark:active:bg-gray-700 tw-dark:active:text-gray-300">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
        ...
        <div class="tw-hidden tw-sm:flex-1 tw-sm:flex tw-sm:items-center tw-sm:justify-end">
            <div>
            ...
            <div>
                <span class="tw-relative tw-z-0 tw-inline-flex tw-rtl:flex-row-reverse tw-shadow-sm tw-rounded-md">
                    ...
                    @else
                        <a  wire:navigate href="{{ $paginator->previousPageUrl() }}" rel="prev" class="tw-relative tw-inline-flex tw-items-center tw-px-2 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-500 tw-bg-white tw-border tw-border-gray-300 tw-rounded-l-md tw-leading-5 tw-hover:text-gray-400 tw-focus:z-10 tw-focus:outline-none tw-focus:ring tw-ring-gray-300 tw-focus:border-blue-300 tw-active:bg-gray-100 tw-active:text-gray-500 tw-transition tw-ease-in-out tw-duration-150 tw-dark:bg-gray-800 tw-dark:border-gray-600 tw-dark:active:bg-gray-700 tw-dark:focus:border-blue-800" aria-label="{{ __('pagination.previous') }}">
                            <svg class="tw-w-5 tw-h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                        </a>
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                ...
                                @else
                                    <a  wire:navigate href="{{ $url }}" class="tw-relative tw-inline-flex tw-items-center tw-px-4 tw-py-2 tw--ml-px tw-text-sm tw-font-medium tw-text-gray-700 tw-bg-white tw-border tw-border-gray-300 tw-leading-5 tw-hover:text-gray-500 tw-focus:z-10 tw-focus:outline-none tw-focus:ring tw-ring-gray-300 tw-focus:border-blue-300 tw-active:bg-gray-100 tw-active:text-gray-700 tw-transition tw-ease-in-out tw-duration-150 tw-dark:bg-gray-800 tw-dark:border-gray-600 tw-dark:text-gray-400 tw-dark:hover:text-gray-300 tw-dark:active:bg-gray-700 tw-dark:focus:border-blue-800" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                    ...
                    {{-- Next Page Link --}}
                    @if ($paginator->hasMorePages())
                        <a  wire:navigate href="{{ $paginator->nextPageUrl() }}" rel="next" class="tw-relative tw-inline-flex tw-items-center tw-px-2 tw-py-2 tw--ml-px tw-text-smtw- font-medium tw-text-gray-500 tw-bg-white tw-border tw-border-gray-300 tw-rounded-r-md tw-leading-5 tw-hover:text-gray-400 tw-focus:z-10 tw-focus:outline-none tw-focus:ring tw-ring-gray-300 tw-focus:border-blue-300 tw-active:bg-gray-100 tw-active:text-gray-500 tw-transition tw-ease-in-out tw-duration-150 tw-dark:bg-gray-800 tw-dark:border-gray-600 tw-dark:active:bg-gray-700 tw-dark:focus:border-blue-800" aria-label="{{ __('pagination.next') }}">
                            <svg class="tw-w-5 tw-h-5" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            <span class="tw-relative tw-inline-flex tw-items-center tw-px-2 tw-py-2 tw--ml-px tw-text-sm tw-font-medium tw-text-gray-500 tw-bg-white border tw-border-gray-300 tw-cursor-default tw-rounded-r-md tw-leading-5 tw-dark:bg-gray-800 tw-dark:border-gray-600" aria-hidden="true">
                                <svg class="tw-w-5 tw-h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
                            </span>
